...
public class RoundCornerImageView extends ImageView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			mRoundWidth = a.getDimensionPixelSize(
					R.styleable.RoundCornerImageView_roundWidth, mRoundWidth);
			mRoundHeight = a.getDimensionPixelSize(
					R.styleable.RoundCornerImageView_roundHeight, mRoundHeight);
			a.recycle();
		} else {
	...
	private void drawLiftDown(Canvas canvas) {
		...
		path.arcTo(new RectF(0, getHeight() - mRoundHeight * 2,
				0 + mRoundWidth * 2, getHeight()), 90, 90);
		path.close();
		canvas.drawPath(path, mPaint);
	}
	...
	private void drawRightDown(Canvas canvas) {
		...
		path.arcTo(new RectF(getWidth() - mRoundWidth * 2, getHeight()
				- mRoundHeight * 2, getWidth(), getHeight()), 0, 90);
		path.close();
		canvas.drawPath(path, mPaint);
	}
	...
	private void drawRightUp(Canvas canvas) {
		...
		path.arcTo(new RectF(getWidth() - mRoundWidth * 2, 0, getWidth(),
				0 + mRoundHeight * 2), -90, 90);
		path.close();
		canvas.drawPath(path, mPaint);
	}
