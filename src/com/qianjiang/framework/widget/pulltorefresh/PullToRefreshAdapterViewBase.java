...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView>
	...
	protected int getItemCount() {
		...
		if (listAdapter == null) {
			...
			itemCount = headerViewListAdapter.getCount()
					- headerViewListAdapter.getFootersCount()
					- headerViewListAdapter.getHeadersCount();
		} else {
			itemCount = listAdapter.getCount();
		}
		EvtLog.d(LOG_TAG, "getIsLoadingData()=" + getIsLoadingData()
	...
	protected void setEmptyViewState() {
		...
		if (null != mEmptyView) {
			...
			if (itemCount > 0) {
				...
				// 非第一次刷新
				if (getFreshSum() != PullToRefreshBase.INITIAL_FRESH_SUM
						&& getIsLoadingData()) {
					mEmptyView.setVisibility(View.GONE);
				} else {
					mEmptyView.setVisibility(View.VISIBLE);
