...
public class PullToRefreshListView extends
	...
	protected final ListView createRefreshableView(Context context,
		...
		mHeaderLoadingView = new HeaderLoadingLayout(context,
		...
		mFooterLoadingView = new FooterLoadingLayout(context,
	...
	protected void setRefreshingInternal(boolean doScroll) {
		...
		// 还有更多数据
		if (originalFooterLoadingLayout != null
				&& originalFooterLoadingLayout.HasMoreData) {
			// Set our Original View to Visible
			if (originalLoadingLayout != null) {
	...
	class InternalListView extends ListView implements EmptyViewMethodAccessor {
		...
		private boolean isMovingInX(float x) {
			final int xDiff = (int) Math.abs(x - mStartX);
			if (xDiff >= mDefaultXDistance / 2) {
				return true;
			}
			return false;
		}

		private boolean isMovingHorizontal(int startX, int startY, int stopX,
				int stopY) {
			int xDiff = stopX - startX;
			int yDiff = Math.abs(stopY - startY);
			boolean conditionOne = xDiff >= mDefaultXDistance
					&& yDiff < mDefaultYDistance;
			boolean conditionTwo = yDiff >= mDefaultYDistance
					&& xDiff < -mDefaultXDistance
