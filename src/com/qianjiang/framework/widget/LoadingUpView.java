...
public class LoadingUpView {
	...
	private void initView() {
		LayoutInflater inflater = (LayoutInflater) mCurrentActivity
		...
		mAnim = AnimationUtils.loadAnimation(mCurrentActivity,
				R.anim.popup_loading);
	}
	...
	private void addLoadingView(String msg) {
		...
		if (mIsFrameLayoutParentView) {
			...
			((FrameLayout) mParentView).addView(mLoadingView,
					new FrameLayout.LayoutParams(mScreenWidth, mScreenHeight
							- mStatusbarHeight));
			if (!mIsLimitView) {
	...
	private void getDisplayConfig() {
		...
		((Activity) mCurrentActivity).getWindowManager().getDefaultDisplay()
				.getMetrics(dm);
		// 宽度
		mScreenWidth = dm.widthPixels;
		mScreenHeight = dm.heightPixels;
		Rect frame = new Rect();
		mCurrentActivity.getWindow().getDecorView()
