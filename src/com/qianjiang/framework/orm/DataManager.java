package com.qianjiang.framework.orm;
...
public abstract class DataManager {
	 ...
	 *            数据库表的描述类
	 */
	protected DataManager(Context context, String dbName, int dbVersion,
	...
	protected void setDatabase() {
		if (mDatabase == null) {
			mDatabase = new Database(mContext, mDBName, mDBVersion,
					mDatabaseBuilder);
		}
	}
	 ...
	 *             数据库访问异常
	 */
	public <T extends BaseModel> T get(Class<T> type, boolean distinct,
			String[] selectColumns, String whereClause, String[] whereArgs,
		...
		Cursor c = null;
		try {
			c = mDatabase.query(false, Utils.getTableName(type), selectColumns,
	...
	public <T extends BaseModel> T get(Class<T> type, String whereClause,
			String[] whereArgs) throws DataAccessException {
		return get(type, false, null, whereClause, whereArgs, null, null, null,
				"1");
	}
	 ...
	 *             数据库访问异常
	 */
	public <T extends BaseModel> List<T> getList(Class<T> type,
			String whereClause, String[] whereArgs) throws DataAccessException {
		return getList(type, false, whereClause, whereArgs, null, null, null,
				null);
	}
	 ...
	 */
	public <T extends BaseModel> List<T> getList(Class<T> type,
			String whereClause, String[] whereArgs, int pageSize, int pageIndex)
			throws DataAccessException {
		return getList(type, false, whereClause, whereArgs, null, null, null,
				pageSize, pageIndex);
	}
	...
	public <T extends BaseModel> List<T> getList(Class<T> type,
			...
			throws DataAccessException {
		return getList(type, false, whereClause, whereArgs, null, null,
				orderBy, null);
	}
	...
	public <T extends BaseModel> List<T> getList(Class<T> type,
		...
		Cursor c = mDatabase.query(distinct, Utils.getTableName(type), null,
				whereClause, whereArgs, groupBy, having, orderBy, limit);
	...
	public <T extends BaseModel> List<T> getList(Class<T> type,
			boolean distinct, String whereClause, String[] whereArgs,
			String groupBy, String having, String orderBy, int pageSize,
	 ...
	 * @throws DataAccessException
	 */
	public <T extends BaseModel, T2 extends BaseModel> List<T> getList(
			boolean distinct, String whereClause, String[] whereArgs,
			String groupBy, String having, String orderBy, String returnCount,
		...
		try {
			...
			for (int i = 0; i < columns.size(); ++i) {
				...
				if (i != columns.size() - 1) {
					returnCols.append(returnTableName).append(POINT)
							.append(colName + DOT);
				} else {
					returnCols.append(returnTableName).append(POINT)
							.append(colName);
	...
	public <T extends BaseModel, T2 extends BaseModel> List<T> getList(
			boolean distinct, String whereClause, String[] whereArgs,
			String groupBy, String having, String orderBy, int pageSize,
			int pageIndex, Class<T> type1, Class<T2> type2)
			throws DataAccessException {
		if (mDatabase == null) {
			throw new DataAccessException(SET_DATABASE_FIRST);
		}
	...
	public long insert(BaseModel model) throws DataAccessException {
		...
		// 关闭时重新打开
		firstOpen();
		List<Field> columns = model.getID() > 0 ? model.getColumnFields()
				: model.getColumnFieldsWithoutID();
		...
		for (Field column : columns) {
			try {
				...
				if (fieldValue != null
						&& (!(typeString.equals("java.util.ArrayList") || typeString
								.equals("java.util.List")) && column.getType()
								.getSuperclass() != BaseModel.class)) {
					values.put(Utils.toSQLName(column.getName()),
							String.valueOf(fieldValue));
				} else if (fieldValue != null
						&& (Utils.isInstanceofBaseModel(column.getType())
								|| typeString.equals("java.util.ArrayList") || typeString
									.equals("java.util.List"))) {
					// 如果该字段继承自BaseModel；也可以保存到数据库中，转化为Json字符串保存
					values.put(Utils.toSQLName(column.getName()),
							new Gson().toJson(fieldValue));
				}
			} catch (IllegalAccessException e) {
				throw new DataAccessException(e.getLocalizedMessage());
			}
	 ...
	 *             数据库访问异常
	 */
	public <T extends BaseModel> int update(BaseModel model)
			throws DataAccessException {
		if (model == null) {
			throw new DataAccessException(OBJECT_CAN_NOT_NULL);
		}
		...
		for (Field column : columns) {
			try {
				...
				throw new DataAccessException(COLUMN_NOT_EXISITS
						+ column.getName());
			} catch (IllegalAccessException e) {
				throw new DataAccessException(COLUMN_NOT_EXISITS
						+ column.getName());
	 ...
	 *             数据库访问异常
	 */
	public <T extends BaseModel> int updateByClause(Class<T> type,
			BaseModel model, String whereClause, String[] whereArgs)
			throws DataAccessException {
		if (model == null) {
			throw new DataAccessException(OBJECT_CAN_NOT_NULL);
		}
	...
	public long save(BaseModel model, String primaryKey, String primaryValue)
			throws DataAccessException {
		if (model == null) {
			throw new DataAccessException(OBJECT_CAN_NOT_NULL);
		}
		...
		firstOpen();
		long id = 0;
		BaseModel existModel = get(model.getClass(), primaryKey,
	...
	public <T extends BaseModel> boolean delete(Class<T> type, long id) {
		// 关闭时重新打开
		firstOpen();
		boolean toRet = delete(type, ID_WHERE_CLAUSE,
	 ...
	 * @return 删除成功后，返回true；否则返回false
	 */
	public <T extends BaseModel> boolean delete(Class<T> type,
			String whereClause, String[] whereArgs) {
		return delete(Utils.getTableName(type), whereClause, whereArgs);
	}

	public <T extends BaseModel> boolean delete(String tableName,
			String whereClause, String[] whereArgs) {
		boolean result = true;
		// 关闭时重新打开
		firstOpen();
