...
public class AppDownloader {
	...
	public void download(final AppInfo appInfo, final double size, final boolean isUpgradeMust) {
		...
		if (isDownloading()) {
			Toast.makeText(QJApplicationBase.CONTEXT, R.string.more_apps_is_downloading, Toast.LENGTH_SHORT).show();
		} else {
			mTmpDownloadThread = new Thread() {
				...
				public void run() {
					...
					File cacheDir;
					try {
						cacheDir = FileUtil.getDownloadDir();
						boolean result = FileUtil.downFile(appInfo.AppUrl, cacheDir.getParent(), cacheDir.getName(),
						...
						String message = mContext.getResources().getString(
								...
								+ e.getMessage();
						Looper.prepare();
						Toast.makeText(QJApplicationBase.CONTEXT, message, Toast.LENGTH_SHORT).show();
						Looper.loop();
						EvtLog.e(TAG, e);
					} finally {
						ISDOWNLOADING = false;
	...
	public void install() {
		try {
			...
			EvtLog.d(TAG, "apk path: " + mFilePath);
	...
	private final class AppDownloadingListener implements FileUtil.OnDownloadingListener {
		...
		public void onDownloading(int progressInByte) {
			...
			EvtLog.d(TAG, progressInByte + "/" + ((int) totalSize) + " " + PROGRESS);
			Message message = new Message();
		...
		public void onError(boolean isUpgradeMust) {
			EvtLog.d(TAG, "onError, " + mDownloadProgressListener);
			if (mDownloadProgressListener != null) {
